...
import { FeatherIconDirective } from "src/app/directives/feather.dirictive";
...
@Component({
  ...
  imports: [...SHARED_IMPORTS, FeatherIconDirective]
})
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.currentThemeMode = (localStorage.getItem('themeMode') ??
      'light') as Mode;
    this.setThemeMode(this.currentThemeMode);

    // this.menuItems = MENU;
    // this.handleMenuItemsForRole();
    this.menuItems = this.handleMenuItemsForRole();
  ...
  setThemeMode(mode: Mode) {
    localStorage.setItem('themeMode', mode ? mode : 'light');
  ...
  // set menu items for sub admin
  handleMenuItemsForRole(): MenuItem[] {
    const allowedPages =
      this.secureLsService.decryptData('pagesAndRoles') ?? [];

    // console.log('================== pages and roles ==================');
    // console.log(allowedPages);
    // console.log(allowedPages.length);

    // console.log('================== all ==================');
    // console.log(MENU);

    if (allowedPages && allowedPages.length) {
      return MENU.filter((route) =>
        allowedPages.some((data: any) => data.pagePath === route.link)
      );
    } else {
      return MENU;
    }
